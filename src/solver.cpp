  ...
  for (int iteration = 0; iteration <= outliers_iterations; iteration++)  //
  {
    ...
    else  //
    {
      std::cout << '\n'
                ...
                << "Right wheel radius: " << res.radius_r
                << std::endl;
    }
...
Eigen::VectorXd cSolver::full_calibration_min(const Eigen::MatrixXd &M) {
  ...
  b = 2 * m13 * m22 * m35 * m15 - pow(m22, 2) * pow(m15, 2) - 2 * m11 * m22 * pow(m35, 2) + 2 * m13 * m22 * m34 * m14 - 2 * m22 * pow(m13, 2) * m44 - pow(m22, 2) * pow(m14, 2) + 2 * m11 * pow(m22, 2) * m44 + pow(m13, 2) * pow(m35, 2) - 2 * m11 * m22 * pow(m34, 2) + pow(m13, 2) * pow(m34, 2);
